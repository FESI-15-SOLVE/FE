...
import { createPostAction, updatePostAction } from '@/actions/post/post-actions';
import {
  createPostSchema,
...
export interface PostFormProps {
  mode: 'create' | 'edit';
  postId?: number;
  initialData?: PostWithComments | null;
}
...
export function PostForm({ mode, postId, initialData }: PostFormProps) {
  ...
  // mode === 'edit' 시 초기 마크다운 내용 에디터에 설정
  useEffect(() => {
    if (editor && mode === 'edit' && initialData?.content) {
      editor.commands.setContent(initialData.content, {
        contentType: 'markdown',
      });
      setValue('content', editor.getText().trim(), { shouldValidate: true });
    }
  }, [editor, mode, initialData?.content, setValue]);
  ...
  const onSubmit = async (data: CreatePostFormValues) => {
    ...
    if (!editor.markdown) {
      toast.error('에디터 초기화 오류입니다. 페이지 새로고침 후 다시 시도해주세요.');
      return;
    }
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        const updatedPost = unwrapAction(
  ...
  return (
    <form
      ...
      className="flex flex-col w-full max-w-[860px] mx-auto gap-6 py-4 sm:py-8 px-3 sm:px-0"
    >
      {/* Title Bar & Submit Button */}
