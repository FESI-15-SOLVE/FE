...
/**
 * Tiptap FileHandler 및 S3 Presigned URL 연동 순수 비동기 업로드 함수
 *
 * Tiptap FileHandler가 건네주는 editor 인스턴스를 파라미터로 직접 전달받아
 * React Ref 의존성 및 Linter 경고 없이 100% 순수하게 동작합니다.
 */
export async function uploadImage(editor: Editor | null, file: File, targetPos?: number) {
  if (!editor) return;
